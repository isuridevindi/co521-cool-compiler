...
Introduction
	...
	of living organisms. (* these (* comments (* are *) nested *) comments *)
 The Rules
...
class CellularAutomaton inherits Board {
 ...
 neighbors(position: Int): Int { 
 	{
	     if north[position] = "X" then 1 else 0 fi     -- invalid token "[" and "]"
...
(* This is where the background pattern is detremined by the user. More 
 ...
 option(): String {
 {
  (let num : Int in
   {
   out_string("\nPlease chose a number: 		--Unterminated string constant
 ...
 prompt() : Bool { 
 {
  (let ans : String in
   {
   out_string("Would you like to continue \0 with the next generation? \n");   --String contains null character
 ...
 prompt2() : Bool { 
  (let ans : String in
   ...
   out_string("Please use lowercase y or n for \			-- to check the escape character
   your answer [n]: ");
...
(* This life.cl is a working program. To check the behavior of the
lexical analyser we have added some errors intentionally. for example:
